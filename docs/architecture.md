...
| `region-selected` | 覆盖层 → 主窗口 | `{ x, y, width, height, mode, annotatedImage? }` | 传递选区坐标（物理像素）或标注后的图片 base64 |
...
快捷键触发
  ...
  → App.tsx 监听 region-selected (mode="screenshot", annotatedImage 存在)
    → copyImageToClipboard(annotatedImage) 复制到剪贴板
    → 主窗口不弹出
```
