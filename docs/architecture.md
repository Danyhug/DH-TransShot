...
| 事件名 | 方向 | 载荷 | 用途 |
|--------|------|------|------|
| `region-selected` | 覆盖层 → 主窗口 | `{ x, y, width, height, mode, monitor_index, annotatedImage? }` | 传递选区坐标（物理像素）或标注后的图片 base64 |
...
快捷键触发
  ...
  → commands/screenshot.rs start_region_select()
    ...
    → 创建覆盖层窗口
  → ScreenshotOverlay 显示冻结截图，用户拖拽选区（悬停展示 HEX 色值，按 C 复制）
  → 选区完成 → 进入标注阶段（前端裁切 + canvas 标注工具栏）
  → 用户标注（矩形/箭头/画笔/文字）→ 确认
  → emit("region-selected") 传递 annotatedImage（base64），关闭覆盖层
  → App.tsx 监听 region-selected (mode="screenshot")
    → 若 annotatedImage 存在：copyImageToClipboard(annotatedImage)
    → 否则：captureRegion(monitor_index, x, y, width, height) → copyImageToClipboard
    → 主窗口不弹出
```
...
快捷键触发
  ...
  → App.tsx 监听 region-selected (mode="ocr_translate")
    → captureRegion(monitor_index, x, y, width, height) 裁切图片
    → captureAndOcr(monitor_index, x, y, width, height, sourceLang) 裁切+OCR 识别
    → setSourceText() 填入源文本
    → translate() 执行翻译
    → 主窗口恢复位置 + show + focus
```
...
- 选区坐标：前端 `ScreenshotOverlay` 在 emit 时按冻结截图实际像素尺寸与覆盖层 CSS 尺寸比例转为物理像素
- 尺寸指示器：显示物理像素尺寸（逻辑尺寸 × 冻结截图/覆盖层比例）
...
pub struct AppState {
    ...
    pub frozen_screenshots: Mutex<Vec<String>>,     // 区域选择期间逐显示器冻结的截图
    pub frozen_mode: Mutex<String>,                 // 区域选择模式
    pub frozen_window_rects: Mutex<serde_json::Value>, // 冻结的窗口矩形列表
...
三个 Zustand store 管理前端状态：

- `translationStore`：源文本、翻译结果、语言选择、翻译状态
- `settingsStore`：服务配置（翻译/OCR/TTS 各自的 model/extra）、设置窗口管理
- `logStore`：日志条目列表、调试窗口管理、`appLog` 辅助函数
