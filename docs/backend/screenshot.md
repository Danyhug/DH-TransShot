...
- `capture_monitors()`、`list_window_rects()` 和 `capture_region_from_full()` 都是**阻塞调用**，必须通过 `spawn_blocking` 在异步上下文中调用
...
- 区域裁切输出为 JPEG（`capture_region_from_full` 和 `capture_region_bytes`），整屏捕获输出为 PNG（`capture_monitors`）
