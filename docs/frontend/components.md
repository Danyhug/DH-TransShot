...
3. 工具栏浮动在选区旁（默认下方，具体定位见「视觉效果（标注阶段）」的三档回退）：矩形/箭头/画笔/马赛克/文字工具切换 + 颜色选择 + 确认/取消
...
- 工具栏：深色半透明圆角条，含工具图标、颜色色块、确认/取消按钮；颜色/线宽按钮为胶囊型，含调色板图标 + 当前线宽/字号数值 + 颜色块
  - 纵向三档回退定位（相对选区容器 top，屏幕 Y = `selRect.top + toolbarTop`）：① 默认紧贴 canvas 下方 → ② 下方溢出屏幕底则翻到 canvas 上方 → ③ 全屏/超高选区上下都放不下时，浮在 canvas 内部底边（半透明覆盖在截图上），确保始终可见
  - 横向：默认与选区左对齐；工具栏右沿溢出窗口时向左回收
